...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      "rounded-[1.75rem] border border-border/70 bg-white/88 text-card-foreground shadow-[0_24px_64px_rgba(7,22,47,0.08)] backdrop-blur-sm",
      className,
    )}
    {...props}
  />
...
const CardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn("text-2xl font-semibold leading-none tracking-[-0.03em]", className)}
      {...props}
    />
  )
);
